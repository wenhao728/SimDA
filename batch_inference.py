...
config = OmegaConf.create(dict(
    ...
    trainable_modules=("temp_adapter", "adapter_s", "adapter_ffn"),
    learning_rate=5e-5,
    n_sample_frames=24,
    fps=12,
    train_batch_size=1,
...
def train_model(
    ...
    train_dataset = SimDADataset(video_path, prompt, n_sample_frames=config.n_sample_frames)
    ...
    for row in tqdm(data_config['data']):
        ...
        for i, edit in tqdm(enumerate(row.edit)):
            ...
            sample = pipeline(
                ...
                video_length=config.n_sample_frames, width=512, height=512,
                num_inference_steps=config.num_inference_steps,
                guidance_scale=config.guidance_scale,
            ).videos
            save_videos_grid(sample, output_dir / f'{i}.gif', fps=config.fps)
